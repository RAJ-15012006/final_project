    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 0' }}>
      <Row gutter={40}>
        ...
        <Col span={14}>
          <div className="waiting-card" style={{
            ...
            <Title level={4} style={{ color: '#00f2ff', marginBottom: '32px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                    {Array.from({ length: settings.players }).map((_, i) => (
                        <div key={i} style={{ textAlign: 'center' }}>
                            <Badge dot={players[i]?.ready} color="#00ff88" offset={[-5, 5]}>
                                <Avatar 
                                    size={64} 
                                    icon={!players[i] ? <UserOutlined /> : null}
                                    style={{ 
                                        background: players[i] ? (players[i].isUser ? '#00f2ff' : '#bc13fe') : 'rgba(255,255,255,0.05)',
                                        border: players[i] ? '2px solid rgba(255,255,255,0.2)' : '2px dashed rgba(255,255,255,0.1)',
                                        color: players[i]?.isUser ? '#000' : '#fff',
                                        fontWeight: 'bold'
                                    }}
                                >
                                    {players[i]?.avatar}
            ...
            <div className="notif-feed" style={{ marginTop: '40px', padding: '15px', background: 'rgba(0,0,0,0.3)', borderLeft: '3px solid #00f2ff', minHeight: '120px' }}>
                {notifs.map((n, i) => (
                    <div key={i} style={{ color: i === 0 ? '#00f2ff' : 'rgba(255,255,255,0.4)', fontSize: '11px', marginBottom: '6px' }}>
                        &gt; {n}
                    </div>
                ))}
            </div>
        ...
        <Col span={10}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div className="waiting-card" style={{ padding: '24px', background: 'rgba(188, 19, 254, 0.05)', border: '1px solid rgba(188, 19, 254, 0.2)', borderRadius: '12px' }}>
                <Text style={{ color: 'rgba(188, 19, 254, 0.6)', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1px' }}>ROOM IDENTIFIER</Text>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '10px' }}>
                    <Title level={2} style={{ color: '#fff', margin: 0, letterSpacing: '4px' }}>{settings.roomCode}</Title>
                    <Button icon={<CopyOutlined />} onClick={copyRoomCode} style={{ color: '#bc13fe', background: 'transparent', border: 'none' }} />
                </div>
            ...
            <Button 
                ...
                style={{
                    height: '80px',
                    width: '100%',
                    background: loading ? 'rgba(255,255,255,0.05)' : 'linear-gradient(45deg, #00f2ff, #bc13fe)',
                    border: 'none',
                    color: '#fff',
                    borderRadius: '8px',
                    fontSize: '20px',
      ...
      <style>{`
        ...
        @keyframes launch-pulse {
            ...
            50% { box-shadow: 0 0 30px rgba(188, 19, 254, 0.6); transform: scale(1.02); }
